...
type HeatmapEvent = Record<string, unknown>;

export async function POST(req: Request) {
  try {
    const body = await req.json();
    const events: HeatmapEvent[] = Array.isArray(body.events) ? body.events : [];
    const dataDir = process.env.HEATMAP_DATA_PATH || path.join(os.tmpdir(), 'pulsevian', 'analytics');
    const outPath = path.join(dataDir, 'heatmaps.ndjson');
